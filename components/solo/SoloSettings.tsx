"use client";

import { useEffect, useState } from "react";
import { Button } from "@/components/Button";
import { cn } from "@/lib/cn";
import { planSummary } from "@/lib/solo/format";
import type { SoloPlan } from "@/lib/solo/types";

type Props = {
  open: boolean;
  plan: SoloPlan;
  onClose: () => void;
  onAssistantToggle: (next: boolean) => void;
  onEditPlan: () => void;
  onReset: () => void;
};

/**
 * Lightweight bottom-sheet drawer. No external dep — just a fixed panel
 * with a soft scrim. Closes on backdrop click or Escape.
 */
export function SoloSettings({
  open,
  plan,
  onClose,
  onAssistantToggle,
  onEditPlan,
  onReset,
}: Props) {
  const [confirmReset, setConfirmReset] = useState(false);

  useEffect(() => {
    if (!open) {
      setConfirmReset(false);
      return;
    }
    function onKey(e: KeyboardEvent) {
      if (e.key === "Escape") onClose();
    }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [open, onClose]);

  return (
    <div
      aria-hidden={!open}
      className={cn(
        "fixed inset-0 z-40 transition-opacity duration-300",
        open ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0",
      )}
    >
      <button
        type="button"
        aria-label="Close settings"
        onClick={onClose}
        className="absolute inset-0 bg-ink/30 backdrop-blur-[2px]"
      />
      <div
        className={cn(
          "absolute inset-x-0 bottom-0 mx-auto w-full max-w-2xl rounded-t-3xl border-t border-line bg-paper px-6 pb-10 pt-6 shadow-soft transition-transform duration-300 ease-calm md:px-10",
          open ? "translate-y-0" : "translate-y-full",
        )}
      >
        <div className="mx-auto mb-6 h-1 w-10 rounded-full bg-line" aria-hidden />
        <p className="text-xs font-light uppercase tracking-[0.25em] text-ink-mute">
          your solo
        </p>
        <h2 className="mt-2 text-xl font-light tracking-brand text-ink">
          {plan.field}
        </h2>
        <p className="mt-1 text-[14px] font-light text-ink-soft">
          {planSummary(plan)}
        </p>

        <div className="mt-8 space-y-5">
          <Row
            label="Quiet assistant"
            hint="Short, calm observations. Off by default."
          >
            <Toggle
              on={plan.assistantEnabled}
              onChange={onAssistantToggle}
              labelOn="On"
              labelOff="Off"
            />
          </Row>

          <Row
            label="Edit plan"
            hint="Change activity, frequency, time, or duration."
          >
            <button
              type="button"
              onClick={onEditPlan}
              className="text-sm font-light text-ink underline-offset-4 hover:underline"
            >
              Edit
            </button>
          </Row>

          <Row label="Reset" hint="Clear plan and history on this device.">
            {confirmReset ? (
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setConfirmReset(false)}
                  className="text-xs font-light uppercase tracking-[0.18em] text-ink-mute hover:text-ink"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={onReset}
                  className="text-xs font-light uppercase tracking-[0.18em] text-ink hover:opacity-70"
                >
                  Confirm reset
                </button>
              </div>
            ) : (
              <button
                type="button"
                onClick={() => setConfirmReset(true)}
                className="text-sm font-light text-ink-soft underline-offset-4 hover:text-ink hover:underline"
              >
                Reset
              </button>
            )}
          </Row>
        </div>

        <div className="mt-10 text-right">
          <Button variant="secondary" size="md" onClick={onClose}>
            Close
          </Button>
        </div>
      </div>
    </div>
  );
}

function Row({
  label,
  hint,
  children,
}: {
  label: string;
  hint: string;
  children: React.ReactNode;
}) {
  return (
    <div className="flex items-start justify-between gap-6 border-t border-line/70 pt-5">
      <div>
        <p className="text-[15px] font-light text-ink">{label}</p>
        <p className="mt-1 max-w-[34ch] text-[13px] font-light leading-relaxed text-ink-mute">
          {hint}
        </p>
      </div>
      <div className="shrink-0">{children}</div>
    </div>
  );
}

function Toggle({
  on,
  onChange,
  labelOn,
  labelOff,
}: {
  on: boolean;
  onChange: (next: boolean) => void;
  labelOn: string;
  labelOff: string;
}) {
  return (
    <button
      type="button"
      role="switch"
      aria-checked={on}
      onClick={() => onChange(!on)}
      className={cn(
        "relative inline-flex h-7 w-12 items-center rounded-full border transition-colors duration-300",
        on ? "border-ink/60 bg-ink" : "border-line bg-surface",
      )}
    >
      <span
        className={cn(
          "absolute top-1/2 inline-block h-5 w-5 -translate-y-1/2 rounded-full transition-transform duration-300 ease-calm",
          on ? "translate-x-6 bg-paper" : "translate-x-1 bg-ink/60",
        )}
      />
      <span className="sr-only">{on ? labelOn : labelOff}</span>
    </button>
  );
}
